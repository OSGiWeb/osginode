...
class PrivateRepository extends Component {
  ...
  showSmartNotification() {
    ...
    if (isCreated !== undefined || isUpdated !== undefined || isDeleted !== undefined) {
      if (isCreated === true) {
        ...
        // this.setState({ showProcessProgress: false }); // TODO: don not use 'setState' in render() function! Use redux to sync., e.g.: 'isProcess' variable
  ...
  /**
   * onDeletePlugin()
   * TODO: use material-ui dialog to handle delete
   */
  onDeletePlugin() {
    const { dispatch } = this.props;
    const { selectedData } = this.props.plugin;
...
// </div>
  ...
  //             <h2>私有插件仓库</h2>
  //             <div className="widget-toolbar">
  //               <button className={classnames(["btn btn-xs btn-primary"]) } onClick={this.handleAddPluginDialogOpen}>
  ...
  //                 </Dropdown.Toggle>
  //                 <Dropdown.Menu className="dropdown-menu pull-right">
  //                   <bsMenuItem onClick={this.handleEditPluginDialogOpen}>
  //                     <i className="fa fa-edit"/>&nbsp; 编辑
  //                   </bsMenuItem>
